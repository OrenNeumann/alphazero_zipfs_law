...
def gather_data(env: str, labels: list[int], max_file_num: int = 1, save_serial: bool = False,
                save_turn_num: bool = False, save_value: bool = False) -> Type[StateCounter]:
    ...
    state_counter = StateCounter(env=env, save_serial=save_serial, save_turn_num=save_turn_num, save_value=save_value,
                                 cut_early_games=False)
    for label in labels:
        num = str(label)
        print("label " + num)
