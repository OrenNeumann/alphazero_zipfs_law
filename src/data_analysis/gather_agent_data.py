...
def gather_data(env: str, labels: list[int], mod=None, max_file_num: int = 1, save_serial: bool = False,
    ...
    path = models_path() + game_path(env) + 'q_'  # '/oware/ur_'
    if mod is None:
        ...
        state_counter = CutoffCounter(env=env, save_serial=save_serial, save_turn_num=save_turn_num,
                                      save_value=save_value, cut_early_games=False)
    else:
        raise ValueError("mod must be None or 'cutoff'")
    for label in labels:
        num = str(label)
        print("label " + num)
