...
from alphazero_scaling.sampling.kl_sampling import Sampler
...
@watcher
def learner(*, game, config, actors, evaluators, broadcast_fn, logger, count_states=False):
  ...
  def learn(step):
    ...
    with open(config.path + '/played_counts.pkl', 'wb') as f:
      pickle.dump([c for _,c in played_counter.most_common()], f)
    with open(config.path + '/sample_counts.pkl', 'wb') as f:
      pickle.dump([c for _,c in sample_counter.most_common()], f)
    ###
    losses = []
...
def alpha_zero(config: Config, count_states=False):###
  ...
  try:
    learner(game=game, config=config, actors=actors,  # pylint: disable=missing-kwoa
            evaluators=evaluators, broadcast_fn=broadcast,count_states=count_states)###
