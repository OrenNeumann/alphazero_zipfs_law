"""
A functionally identical version of alpha_zero.py, turned into a class.
This is a base from which one can easily introduce changes to the algorithm. Inherit AlphaZero 
and override methods to change the algorithm at any stage.
...
class AlphaZero(object):
    ...
    def evaluator(self, *, logger, queue):
        ...
        for game_num in itertools.count():
            ...
            bots = [
                self._init_bot(self.config, self.game, az_evaluator, True),
                mcts.MCTSBot(
                    self.game,
                    self.config.uct_c,
                    max_simulations,
